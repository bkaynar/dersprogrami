import { queryParams, type RouteQueryOptions, type RouteDefinition, type RouteFormDefinition, applyUrlDefaults } from './../../../../wayfinder'
/**
* @see \App\Http\Controllers\OgretmenDersController::index
* @see app/Http/Controllers/OgretmenDersController.php:13
* @route '/ogretmen-dersleri'
*/
export const index = (options?: RouteQueryOptions): RouteDefinition<'get'> => ({
    url: index.url(options),
    method: 'get',
})

index.definition = {
    methods: ["get","head"],
    url: '/ogretmen-dersleri',
} satisfies RouteDefinition<["get","head"]>

/**
* @see \App\Http\Controllers\OgretmenDersController::index
* @see app/Http/Controllers/OgretmenDersController.php:13
* @route '/ogretmen-dersleri'
*/
index.url = (options?: RouteQueryOptions) => {
    return index.definition.url + queryParams(options)
}

/**
* @see \App\Http\Controllers\OgretmenDersController::index
* @see app/Http/Controllers/OgretmenDersController.php:13
* @route '/ogretmen-dersleri'
*/
index.get = (options?: RouteQueryOptions): RouteDefinition<'get'> => ({
    url: index.url(options),
    method: 'get',
})

/**
* @see \App\Http\Controllers\OgretmenDersController::index
* @see app/Http/Controllers/OgretmenDersController.php:13
* @route '/ogretmen-dersleri'
*/
index.head = (options?: RouteQueryOptions): RouteDefinition<'head'> => ({
    url: index.url(options),
    method: 'head',
})

/**
* @see \App\Http\Controllers\OgretmenDersController::index
* @see app/Http/Controllers/OgretmenDersController.php:13
* @route '/ogretmen-dersleri'
*/
const indexForm = (options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: index.url(options),
    method: 'get',
})

/**
* @see \App\Http\Controllers\OgretmenDersController::index
* @see app/Http/Controllers/OgretmenDersController.php:13
* @route '/ogretmen-dersleri'
*/
indexForm.get = (options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: index.url(options),
    method: 'get',
})

/**
* @see \App\Http\Controllers\OgretmenDersController::index
* @see app/Http/Controllers/OgretmenDersController.php:13
* @route '/ogretmen-dersleri'
*/
indexForm.head = (options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: index.url({
        [options?.mergeQuery ? 'mergeQuery' : 'query']: {
            _method: 'HEAD',
            ...(options?.query ?? options?.mergeQuery ?? {}),
        }
    }),
    method: 'get',
})

index.form = indexForm

/**
* @see \App\Http\Controllers\OgretmenDersController::create
* @see app/Http/Controllers/OgretmenDersController.php:28
* @route '/ogretmen-dersleri/create'
*/
export const create = (options?: RouteQueryOptions): RouteDefinition<'get'> => ({
    url: create.url(options),
    method: 'get',
})

create.definition = {
    methods: ["get","head"],
    url: '/ogretmen-dersleri/create',
} satisfies RouteDefinition<["get","head"]>

/**
* @see \App\Http\Controllers\OgretmenDersController::create
* @see app/Http/Controllers/OgretmenDersController.php:28
* @route '/ogretmen-dersleri/create'
*/
create.url = (options?: RouteQueryOptions) => {
    return create.definition.url + queryParams(options)
}

/**
* @see \App\Http\Controllers\OgretmenDersController::create
* @see app/Http/Controllers/OgretmenDersController.php:28
* @route '/ogretmen-dersleri/create'
*/
create.get = (options?: RouteQueryOptions): RouteDefinition<'get'> => ({
    url: create.url(options),
    method: 'get',
})

/**
* @see \App\Http\Controllers\OgretmenDersController::create
* @see app/Http/Controllers/OgretmenDersController.php:28
* @route '/ogretmen-dersleri/create'
*/
create.head = (options?: RouteQueryOptions): RouteDefinition<'head'> => ({
    url: create.url(options),
    method: 'head',
})

/**
* @see \App\Http\Controllers\OgretmenDersController::create
* @see app/Http/Controllers/OgretmenDersController.php:28
* @route '/ogretmen-dersleri/create'
*/
const createForm = (options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: create.url(options),
    method: 'get',
})

/**
* @see \App\Http\Controllers\OgretmenDersController::create
* @see app/Http/Controllers/OgretmenDersController.php:28
* @route '/ogretmen-dersleri/create'
*/
createForm.get = (options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: create.url(options),
    method: 'get',
})

/**
* @see \App\Http\Controllers\OgretmenDersController::create
* @see app/Http/Controllers/OgretmenDersController.php:28
* @route '/ogretmen-dersleri/create'
*/
createForm.head = (options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: create.url({
        [options?.mergeQuery ? 'mergeQuery' : 'query']: {
            _method: 'HEAD',
            ...(options?.query ?? options?.mergeQuery ?? {}),
        }
    }),
    method: 'get',
})

create.form = createForm

/**
* @see \App\Http\Controllers\OgretmenDersController::store
* @see app/Http/Controllers/OgretmenDersController.php:39
* @route '/ogretmen-dersleri'
*/
export const store = (options?: RouteQueryOptions): RouteDefinition<'post'> => ({
    url: store.url(options),
    method: 'post',
})

store.definition = {
    methods: ["post"],
    url: '/ogretmen-dersleri',
} satisfies RouteDefinition<["post"]>

/**
* @see \App\Http\Controllers\OgretmenDersController::store
* @see app/Http/Controllers/OgretmenDersController.php:39
* @route '/ogretmen-dersleri'
*/
store.url = (options?: RouteQueryOptions) => {
    return store.definition.url + queryParams(options)
}

/**
* @see \App\Http\Controllers\OgretmenDersController::store
* @see app/Http/Controllers/OgretmenDersController.php:39
* @route '/ogretmen-dersleri'
*/
store.post = (options?: RouteQueryOptions): RouteDefinition<'post'> => ({
    url: store.url(options),
    method: 'post',
})

/**
* @see \App\Http\Controllers\OgretmenDersController::store
* @see app/Http/Controllers/OgretmenDersController.php:39
* @route '/ogretmen-dersleri'
*/
const storeForm = (options?: RouteQueryOptions): RouteFormDefinition<'post'> => ({
    action: store.url(options),
    method: 'post',
})

/**
* @see \App\Http\Controllers\OgretmenDersController::store
* @see app/Http/Controllers/OgretmenDersController.php:39
* @route '/ogretmen-dersleri'
*/
storeForm.post = (options?: RouteQueryOptions): RouteFormDefinition<'post'> => ({
    action: store.url(options),
    method: 'post',
})

store.form = storeForm

/**
* @see \App\Http\Controllers\OgretmenDersController::show
* @see app/Http/Controllers/OgretmenDersController.php:61
* @route '/ogretmen-dersleri/{ogretmen}'
*/
export const show = (args: { ogretmen: string | number } | [ogretmen: string | number ] | string | number, options?: RouteQueryOptions): RouteDefinition<'get'> => ({
    url: show.url(args, options),
    method: 'get',
})

show.definition = {
    methods: ["get","head"],
    url: '/ogretmen-dersleri/{ogretmen}',
} satisfies RouteDefinition<["get","head"]>

/**
* @see \App\Http\Controllers\OgretmenDersController::show
* @see app/Http/Controllers/OgretmenDersController.php:61
* @route '/ogretmen-dersleri/{ogretmen}'
*/
show.url = (args: { ogretmen: string | number } | [ogretmen: string | number ] | string | number, options?: RouteQueryOptions) => {
    if (typeof args === 'string' || typeof args === 'number') {
        args = { ogretmen: args }
    }

    if (Array.isArray(args)) {
        args = {
            ogretmen: args[0],
        }
    }

    args = applyUrlDefaults(args)

    const parsedArgs = {
        ogretmen: args.ogretmen,
    }

    return show.definition.url
            .replace('{ogretmen}', parsedArgs.ogretmen.toString())
            .replace(/\/+$/, '') + queryParams(options)
}

/**
* @see \App\Http\Controllers\OgretmenDersController::show
* @see app/Http/Controllers/OgretmenDersController.php:61
* @route '/ogretmen-dersleri/{ogretmen}'
*/
show.get = (args: { ogretmen: string | number } | [ogretmen: string | number ] | string | number, options?: RouteQueryOptions): RouteDefinition<'get'> => ({
    url: show.url(args, options),
    method: 'get',
})

/**
* @see \App\Http\Controllers\OgretmenDersController::show
* @see app/Http/Controllers/OgretmenDersController.php:61
* @route '/ogretmen-dersleri/{ogretmen}'
*/
show.head = (args: { ogretmen: string | number } | [ogretmen: string | number ] | string | number, options?: RouteQueryOptions): RouteDefinition<'head'> => ({
    url: show.url(args, options),
    method: 'head',
})

/**
* @see \App\Http\Controllers\OgretmenDersController::show
* @see app/Http/Controllers/OgretmenDersController.php:61
* @route '/ogretmen-dersleri/{ogretmen}'
*/
const showForm = (args: { ogretmen: string | number } | [ogretmen: string | number ] | string | number, options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: show.url(args, options),
    method: 'get',
})

/**
* @see \App\Http\Controllers\OgretmenDersController::show
* @see app/Http/Controllers/OgretmenDersController.php:61
* @route '/ogretmen-dersleri/{ogretmen}'
*/
showForm.get = (args: { ogretmen: string | number } | [ogretmen: string | number ] | string | number, options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: show.url(args, options),
    method: 'get',
})

/**
* @see \App\Http\Controllers\OgretmenDersController::show
* @see app/Http/Controllers/OgretmenDersController.php:61
* @route '/ogretmen-dersleri/{ogretmen}'
*/
showForm.head = (args: { ogretmen: string | number } | [ogretmen: string | number ] | string | number, options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: show.url(args, {
        [options?.mergeQuery ? 'mergeQuery' : 'query']: {
            _method: 'HEAD',
            ...(options?.query ?? options?.mergeQuery ?? {}),
        }
    }),
    method: 'get',
})

show.form = showForm

/**
* @see \App\Http\Controllers\OgretmenDersController::edit
* @see app/Http/Controllers/OgretmenDersController.php:81
* @route '/ogretmen-dersleri/{ogretmen}/edit'
*/
export const edit = (args: { ogretmen: string | number } | [ogretmen: string | number ] | string | number, options?: RouteQueryOptions): RouteDefinition<'get'> => ({
    url: edit.url(args, options),
    method: 'get',
})

edit.definition = {
    methods: ["get","head"],
    url: '/ogretmen-dersleri/{ogretmen}/edit',
} satisfies RouteDefinition<["get","head"]>

/**
* @see \App\Http\Controllers\OgretmenDersController::edit
* @see app/Http/Controllers/OgretmenDersController.php:81
* @route '/ogretmen-dersleri/{ogretmen}/edit'
*/
edit.url = (args: { ogretmen: string | number } | [ogretmen: string | number ] | string | number, options?: RouteQueryOptions) => {
    if (typeof args === 'string' || typeof args === 'number') {
        args = { ogretmen: args }
    }

    if (Array.isArray(args)) {
        args = {
            ogretmen: args[0],
        }
    }

    args = applyUrlDefaults(args)

    const parsedArgs = {
        ogretmen: args.ogretmen,
    }

    return edit.definition.url
            .replace('{ogretmen}', parsedArgs.ogretmen.toString())
            .replace(/\/+$/, '') + queryParams(options)
}

/**
* @see \App\Http\Controllers\OgretmenDersController::edit
* @see app/Http/Controllers/OgretmenDersController.php:81
* @route '/ogretmen-dersleri/{ogretmen}/edit'
*/
edit.get = (args: { ogretmen: string | number } | [ogretmen: string | number ] | string | number, options?: RouteQueryOptions): RouteDefinition<'get'> => ({
    url: edit.url(args, options),
    method: 'get',
})

/**
* @see \App\Http\Controllers\OgretmenDersController::edit
* @see app/Http/Controllers/OgretmenDersController.php:81
* @route '/ogretmen-dersleri/{ogretmen}/edit'
*/
edit.head = (args: { ogretmen: string | number } | [ogretmen: string | number ] | string | number, options?: RouteQueryOptions): RouteDefinition<'head'> => ({
    url: edit.url(args, options),
    method: 'head',
})

/**
* @see \App\Http\Controllers\OgretmenDersController::edit
* @see app/Http/Controllers/OgretmenDersController.php:81
* @route '/ogretmen-dersleri/{ogretmen}/edit'
*/
const editForm = (args: { ogretmen: string | number } | [ogretmen: string | number ] | string | number, options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: edit.url(args, options),
    method: 'get',
})

/**
* @see \App\Http\Controllers\OgretmenDersController::edit
* @see app/Http/Controllers/OgretmenDersController.php:81
* @route '/ogretmen-dersleri/{ogretmen}/edit'
*/
editForm.get = (args: { ogretmen: string | number } | [ogretmen: string | number ] | string | number, options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: edit.url(args, options),
    method: 'get',
})

/**
* @see \App\Http\Controllers\OgretmenDersController::edit
* @see app/Http/Controllers/OgretmenDersController.php:81
* @route '/ogretmen-dersleri/{ogretmen}/edit'
*/
editForm.head = (args: { ogretmen: string | number } | [ogretmen: string | number ] | string | number, options?: RouteQueryOptions): RouteFormDefinition<'get'> => ({
    action: edit.url(args, {
        [options?.mergeQuery ? 'mergeQuery' : 'query']: {
            _method: 'HEAD',
            ...(options?.query ?? options?.mergeQuery ?? {}),
        }
    }),
    method: 'get',
})

edit.form = editForm

/**
* @see \App\Http\Controllers\OgretmenDersController::update
* @see app/Http/Controllers/OgretmenDersController.php:101
* @route '/ogretmen-dersleri/{ogretmen}'
*/
export const update = (args: { ogretmen: string | number } | [ogretmen: string | number ] | string | number, options?: RouteQueryOptions): RouteDefinition<'put'> => ({
    url: update.url(args, options),
    method: 'put',
})

update.definition = {
    methods: ["put"],
    url: '/ogretmen-dersleri/{ogretmen}',
} satisfies RouteDefinition<["put"]>

/**
* @see \App\Http\Controllers\OgretmenDersController::update
* @see app/Http/Controllers/OgretmenDersController.php:101
* @route '/ogretmen-dersleri/{ogretmen}'
*/
update.url = (args: { ogretmen: string | number } | [ogretmen: string | number ] | string | number, options?: RouteQueryOptions) => {
    if (typeof args === 'string' || typeof args === 'number') {
        args = { ogretmen: args }
    }

    if (Array.isArray(args)) {
        args = {
            ogretmen: args[0],
        }
    }

    args = applyUrlDefaults(args)

    const parsedArgs = {
        ogretmen: args.ogretmen,
    }

    return update.definition.url
            .replace('{ogretmen}', parsedArgs.ogretmen.toString())
            .replace(/\/+$/, '') + queryParams(options)
}

/**
* @see \App\Http\Controllers\OgretmenDersController::update
* @see app/Http/Controllers/OgretmenDersController.php:101
* @route '/ogretmen-dersleri/{ogretmen}'
*/
update.put = (args: { ogretmen: string | number } | [ogretmen: string | number ] | string | number, options?: RouteQueryOptions): RouteDefinition<'put'> => ({
    url: update.url(args, options),
    method: 'put',
})

/**
* @see \App\Http\Controllers\OgretmenDersController::update
* @see app/Http/Controllers/OgretmenDersController.php:101
* @route '/ogretmen-dersleri/{ogretmen}'
*/
const updateForm = (args: { ogretmen: string | number } | [ogretmen: string | number ] | string | number, options?: RouteQueryOptions): RouteFormDefinition<'post'> => ({
    action: update.url(args, {
        [options?.mergeQuery ? 'mergeQuery' : 'query']: {
            _method: 'PUT',
            ...(options?.query ?? options?.mergeQuery ?? {}),
        }
    }),
    method: 'post',
})

/**
* @see \App\Http\Controllers\OgretmenDersController::update
* @see app/Http/Controllers/OgretmenDersController.php:101
* @route '/ogretmen-dersleri/{ogretmen}'
*/
updateForm.put = (args: { ogretmen: string | number } | [ogretmen: string | number ] | string | number, options?: RouteQueryOptions): RouteFormDefinition<'post'> => ({
    action: update.url(args, {
        [options?.mergeQuery ? 'mergeQuery' : 'query']: {
            _method: 'PUT',
            ...(options?.query ?? options?.mergeQuery ?? {}),
        }
    }),
    method: 'post',
})

update.form = updateForm

/**
* @see \App\Http\Controllers\OgretmenDersController::destroy
* @see app/Http/Controllers/OgretmenDersController.php:123
* @route '/ogretmen-dersleri/{ogretmenId}/{dersId}'
*/
export const destroy = (args: { ogretmenId: string | number, dersId: string | number } | [ogretmenId: string | number, dersId: string | number ], options?: RouteQueryOptions): RouteDefinition<'delete'> => ({
    url: destroy.url(args, options),
    method: 'delete',
})

destroy.definition = {
    methods: ["delete"],
    url: '/ogretmen-dersleri/{ogretmenId}/{dersId}',
} satisfies RouteDefinition<["delete"]>

/**
* @see \App\Http\Controllers\OgretmenDersController::destroy
* @see app/Http/Controllers/OgretmenDersController.php:123
* @route '/ogretmen-dersleri/{ogretmenId}/{dersId}'
*/
destroy.url = (args: { ogretmenId: string | number, dersId: string | number } | [ogretmenId: string | number, dersId: string | number ], options?: RouteQueryOptions) => {
    if (Array.isArray(args)) {
        args = {
            ogretmenId: args[0],
            dersId: args[1],
        }
    }

    args = applyUrlDefaults(args)

    const parsedArgs = {
        ogretmenId: args.ogretmenId,
        dersId: args.dersId,
    }

    return destroy.definition.url
            .replace('{ogretmenId}', parsedArgs.ogretmenId.toString())
            .replace('{dersId}', parsedArgs.dersId.toString())
            .replace(/\/+$/, '') + queryParams(options)
}

/**
* @see \App\Http\Controllers\OgretmenDersController::destroy
* @see app/Http/Controllers/OgretmenDersController.php:123
* @route '/ogretmen-dersleri/{ogretmenId}/{dersId}'
*/
destroy.delete = (args: { ogretmenId: string | number, dersId: string | number } | [ogretmenId: string | number, dersId: string | number ], options?: RouteQueryOptions): RouteDefinition<'delete'> => ({
    url: destroy.url(args, options),
    method: 'delete',
})

/**
* @see \App\Http\Controllers\OgretmenDersController::destroy
* @see app/Http/Controllers/OgretmenDersController.php:123
* @route '/ogretmen-dersleri/{ogretmenId}/{dersId}'
*/
const destroyForm = (args: { ogretmenId: string | number, dersId: string | number } | [ogretmenId: string | number, dersId: string | number ], options?: RouteQueryOptions): RouteFormDefinition<'post'> => ({
    action: destroy.url(args, {
        [options?.mergeQuery ? 'mergeQuery' : 'query']: {
            _method: 'DELETE',
            ...(options?.query ?? options?.mergeQuery ?? {}),
        }
    }),
    method: 'post',
})

/**
* @see \App\Http\Controllers\OgretmenDersController::destroy
* @see app/Http/Controllers/OgretmenDersController.php:123
* @route '/ogretmen-dersleri/{ogretmenId}/{dersId}'
*/
destroyForm.delete = (args: { ogretmenId: string | number, dersId: string | number } | [ogretmenId: string | number, dersId: string | number ], options?: RouteQueryOptions): RouteFormDefinition<'post'> => ({
    action: destroy.url(args, {
        [options?.mergeQuery ? 'mergeQuery' : 'query']: {
            _method: 'DELETE',
            ...(options?.query ?? options?.mergeQuery ?? {}),
        }
    }),
    method: 'post',
})

destroy.form = destroyForm

const OgretmenDersController = { index, create, store, show, edit, update, destroy }

export default OgretmenDersController