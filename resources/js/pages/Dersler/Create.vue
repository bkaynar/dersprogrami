...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    {
        title: 'Yeni Ders',
        href: derslerRoute.create().url,
    },
];

const submit = () => {
  form.post(derslerRoute.store().url);
};
</script>

<template>
  <Head title="Yeni Ders" />
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="p-6">
      ...
      <div class="mb-6 flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-semibold">Yeni Ders Oluştur</h1>
          <p class="mt-1 text-sm text-muted-foreground">
            Gerekli bilgileri doldurarak yeni bir ders ekleyin
          </p>
        </div>
        <Link
          :href="derslerRoute.index().url"
        ...
        <form @submit.prevent="submit" class="space-y-6">
          ...
          <div class="flex items-center gap-3 pt-4">
            ...
            >
              ...
              {{ form.processing ? 'Oluşturuluyor...' : 'Dersi Oluştur' }}
            </button>
            <Link
              :href="derslerRoute.index().url"
              class="inline-flex items-center gap-2 rounded-lg border bg-card px-4 py-2 text-sm font-medium hover:bg-accent"
