...
const form = useForm({
  ogrenci_grup_id: props.kisitlama.ogrenci_grup_id,
  zaman_dilimi_id: props.kisitlama.zaman_dilimi_id,
  musait_mi: props.kisitlama.musait_mi,
});
...
const submit = () => {
  form.put(`/grup-kisitlamalari/${props.kisitlama.id}`);
};
</script>

<template>
  <Head title="Kısıtlama Düzenle" />
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="p-6">
      ...
      <div class="mb-6 flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-semibold">Kısıtlama Düzenle</h1>
          <p class="mt-1 text-sm text-muted-foreground">
            Grup kısıtlama bilgilerini güncelleyin
          </p>
        </div>
        <Link
          href="/grup-kisitlamalari"
        ...
        <form @submit.prevent="submit" class="space-y-6">
          ...
          <div class="flex items-center gap-3 pt-4">
            ...
            >
              ...
              {{ form.processing ? 'Güncelleniyor...' : 'Kısıtlamayı Güncelle' }}
            </button>
            <Link
              href="/grup-kisitlamalari"
              class="inline-flex items-center gap-2 rounded-lg border bg-card px-4 py-2 text-sm font-medium hover:bg-accent"
