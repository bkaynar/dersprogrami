...
const form = useForm({
  isim: props.ogrenci_grubu.isim,
  yil: props.ogrenci_grubu.yil,
  ogrenci_sayisi: props.ogrenci_grubu.ogrenci_sayisi,
  ust_grup_id: props.ogrenci_grubu.ust_grup_id || null,
});
...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    {
        title: 'Düzenle',
        href: `/ogrenci-gruplari/${props.ogrenci_grubu.id}/edit`,
    },
];

const submit = () => {
  form.put(`/ogrenci-gruplari/${props.ogrenci_grubu.id}`);
};
</script>

<template>
  <Head :title="`${ogrenci_grubu.isim} - Düzenle`" />
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="p-6">
      ...
      <div class="mb-6 flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-semibold">Öğrenci Grubu Düzenle</h1>
          <p class="mt-1 text-sm text-muted-foreground">
            {{ ogrenci_grubu.isim }} grubunun bilgilerini güncelleyin
          </p>
        </div>
        <Link
          :href="`/ogrenci-gruplari/${ogrenci_grubu.id}`"
