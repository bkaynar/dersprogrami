...
const downloadTemplate = () => {
    window.location.href = '/ogrenci-gruplari/template/download';
};

const downloadExcel = () => {
    window.location.href = '/ogrenci-gruplari/export';
};
...
const uploadFile = () => {
    ...
    importForm.post('/ogrenci-gruplari/import', {
        forceFormData: true,
        onSuccess: () => {
            importForm.reset();
            if (fileInput.value) {
            ...
            <div class="mb-6 flex items-center justify-between">
                ...
                <div class="flex items-center gap-2">
                    ...
                    <button
                        @click="downloadExcel"
                        class="inline-flex items-center gap-2 rounded-lg border bg-background px-4 py-2 text-sm font-medium hover:bg-muted"
                        title="Öğrenci Gruplarını Excel olarak indir"
                    >
                        <svg class="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
