...
const form = useForm({
  isim: props.mekan.isim,
  mekan_tipi: props.mekan.mekan_tipi,
  kapasite: props.mekan.kapasite,
});
...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    {
        title: 'Düzenle',
        href: `/mekanlar/${props.mekan.id}/edit`,
    },
];

const submit = () => {
  form.put(`/mekanlar/${props.mekan.id}`);
};
</script>

<template>
  <Head :title="`${mekan.isim} - Düzenle`" />
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="p-6">
      ...
      <div class="mb-6 flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-semibold">Mekan Düzenle</h1>
          <p class="mt-1 text-sm text-muted-foreground">
            {{ mekan.isim }} mekanının bilgilerini güncelleyin
          </p>
        </div>
        <Link
