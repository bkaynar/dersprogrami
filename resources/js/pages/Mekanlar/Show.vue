...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    {
        title: props.mekan.isim,
        href: `/mekanlar/${props.mekan.id}`,
    },
];
...
const deleteMekan = () => {
    ...
    router.delete(`/mekanlar/${props.mekan.id}`, {
        onFinish: () => {
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="p-6">
      ...
      <div class="mb-6 flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-semibold">{{ mekan.isim }}</h1>
          <p class="mt-1 text-sm text-muted-foreground">
            Mekan detaylarını görüntüleyin
          </p>
        </div>
      ...
      <div class="max-w-2xl rounded-lg border bg-card p-6">
        <div class="space-y-6">
          <div>
            <h3 class="text-sm font-medium text-muted-foreground">Mekan Adı</h3>
            <p class="mt-1 text-lg font-medium">{{ mekan.isim }}</p>
          </div>

          <div class="grid grid-cols-2 gap-6">
            <div>
              <h3 class="text-sm font-medium text-muted-foreground">Mekan Tipi</h3>
              <p class="mt-1">
            ...
            <div>
              <h3 class="text-sm font-medium text-muted-foreground">Kapasite</h3>
              <p class="mt-1 text-lg font-medium">{{ mekan.kapasite }} kişi</p>
            </div>
          </div>
          ...
          <div class="border-t pt-6">
            <div class="grid grid-cols-2 gap-6">
              <div>
                ...
                <p class="mt-1 text-sm">{{ new Date(mekan.created_at).toLocaleDateString('tr-TR') }}</p>
              </div>

              <div>
                <h3 class="text-sm font-medium text-muted-foreground">Son Güncelleme</h3>
                <p class="mt-1 text-sm">{{ new Date(mekan.updated_at).toLocaleDateString('tr-TR') }}</p>
              </div>
            </div>
          </div>
        </div>
    ...
    <div v-if="showDeleteDialog" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div class="w-full max-w-md rounded-lg border bg-card p-6 shadow-lg">
        <h2 class="text-xl font-semibold">Mekanı Sil</h2>
        <p class="mt-2 text-sm text-muted-foreground">
          <strong>{{ mekan.isim }}</strong> mekanını silmek istediğinizden emin misiniz? Bu işlem geri alınamaz.
        </p>
