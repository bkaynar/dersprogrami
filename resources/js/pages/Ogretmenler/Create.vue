...
const submit = () => {
  form.post(ogretmenler.store().url);
};
</script>

<template>
  <Head title="Yeni Öğretmen" />
  ...
  <AppLayout :breadcrumbs="breadcrumbs">
    <div class="p-6">
      ...
      <div class="mb-6 flex items-center justify-between">
        <div>
          <h1 class="text-2xl font-semibold">Yeni Öğretmen Oluştur</h1>
          <p class="mt-1 text-sm text-muted-foreground">
            Gerekli bilgileri doldurarak yeni bir öğretmen ekleyin
          </p>
        </div>
        <Link
          :href="ogretmenler.index().url"
        ...
        <form @submit.prevent="submit" class="space-y-6">
          ...
          <div class="flex items-center gap-3 pt-4">
            ...
            >
              ...
              {{ form.processing ? 'Oluşturuluyor...' : 'Öğretmeni Oluştur' }}
            </button>
            <Link
              :href="ogretmenler.index().url"
              class="inline-flex items-center gap-2 rounded-lg border bg-card px-4 py-2 text-sm font-medium hover:bg-accent"
